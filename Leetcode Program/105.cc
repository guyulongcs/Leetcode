...
public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        ...
        T = buildTreeRange(preorder, inorder, 0, N-1, 0, N-1);
        return T;
    }
    
    TreeNode * buildTreeRange(vector<int>& preorder, vector<int>& inorder, int pb, int pe, int ib, int ie) {
        ...
        T = new TreeNode(r);
        T->left = buildTreeRange(preorder, inorder, pb+1, pb+ir-ib, ib, ir-1);
        T->right = buildTreeRange(preorder, inorder, pe-ie+ir+1 ,pe,ir+1, ie);
        return T;
    }
...
int main() {
    ...
    Solution s;
    vector<int> vp({1,2,4,5,3});
    vector<int> vi({4,2,5,1,3});

    TreeNode * T = s.buildTree(vp, vi);
    Tree::printTree(T);
